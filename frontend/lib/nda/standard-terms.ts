 ...
 * Canonical text: https://commonpaper.com/standards/mutual-nda/1.0
 *
 * The clause text is reproduced verbatim from `templates/mutual-nda.md` with two
 * kinds of markup added:
 *
 *   `**bold**`   — defined terms, as in the source markdown.
 *   `{{token}}`  — a reference to a Cover Page field. In the source these are
 *                  `<span class="coverpage_link">` links back to the cover page;
 *                  here they resolve against the user's answers (see `render.ts`).
 *
 * Where the source repeats a reference within a sentence, the second occurrence is
 * written as the anaphor a filled-in agreement would actually use ("such State",
 * "such courts") rather than a second token, so the prose stays grammatical.
 */

export interface Clause {
  /** 1-based number as it appears in the agreement. */
  number: number;
  /** Bolded lead-in, e.g. "Introduction". */
  heading: string;
  body: string;
}
...
export const STANDARD_TERMS: readonly Clause[] = [
  {
    number: 1,
