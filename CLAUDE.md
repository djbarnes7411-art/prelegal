...
- Run it with the `scripts/` above; run the tests with `cd backend && uv run pytest` (306) and `cd frontend && npm test` (335).
...
- Autosave is debounced a second after the last change, coalescing a turn's document and transcript updates into one request, and flushes on unmount so leaving the page does not drop the last turn. A failed save is a quiet mark in the top bar and nothing more — the chat's error bubble and its focus handling are reserved for turns that failed, and the next turn retries the save anyway.
